...
class OAS(http.Controller):
    ...
    @http.route('/api/rush_token/<api_key>/<token>', type='json',  methods=['POST'], auth='none', csrf=False)
    def get_token(self, api_key, token, **kw):
        #user = http.request.env['res.users'].sudo().search([('login', '=', api_key), ('token', '=', token)], limit=1)
        user = http.request.env['res.users'].sudo().search([('login', '=', api_key)], limit=1)
        if user:
            user.reset_token()
            return {'new_token': user.token, 'error_code': 0, 'error_message': ''}
        else:
            return {'error_code': '1', 'error_message': '认证错误'}


    @http.route('/api/v1/<namespace_name>/swagger.json', type='http', auth='none', csrf=False, cors='*')
